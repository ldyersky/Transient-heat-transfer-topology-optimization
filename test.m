...
% J3 = T'*P1*T
% 
% % xPhys = 0.25*ones(length(element(:,1)),1);
% % % xPhys(350) = xPhys(1) + dlta;
% % [KT,C] = matrixKC(node,element,k,rho,cp,xPhys,penal1,penal2);
% % H = matrixH(element,node,node_h,h);
% % Hc = H(freedofs,freedofs);
% % K = KT+H;
% % Kc = K(freedofs,freedofs);
% % Cc = C(freedofs,freedofs);
% % Q2 = Kc;
% % TT = 100*ismember(AllDofs,Fixnodes1);
% % T = TT';
% % T = T(freedofs,1);
% % T0 = -Kc\Hc*T;
% % Kc_inv = inv(Kc);
